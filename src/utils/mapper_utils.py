...
def compute_opt_views_distribution(keyframes_num, iterations_num, current_frame_iter) -> np.ndarray:
    ...
    if keyframes_num == 1:
        return np.array([1.0])
    prob = np.full(keyframes_num, (iterations_num - current_frame_iter) / (keyframes_num - 1)) #np.full（形状，值），初始化一个np
...
def compute_camera_frustum_corners(depth_map: np.ndarray, pose: np.ndarray, intrinsics: np.ndarray) -> np.ndarray:
    ...
    Args:
        ...
        pose: The camera pose matrix.（c2w）
    ...
    corners = np.array( #视锥体的立方体8个角点 形状：[8,3]
        [
            [0, 0, min_depth], #[u,v,depth]
            [width, 0, min_depth],
            [0, height, min_depth],
    ...
    x = (corners[:, 0] - intrinsics[0, 2]) * corners[:, 2] / intrinsics[0, 0] #将8个立方体角点转换到相机坐标系下的视锥体
...
def compute_frustum_point_ids(pts: torch.Tensor, frustum_corners: torch.Tensor, device: str = "cuda"):
    ...
    # Broad phase 用xyz最小和最大的两个点组成的立方体粗裁减
    pts = pts.to(device)
...
def sample_pixels_based_on_gradient(image: np.ndarray, num_samples: int) -> np.ndarray:
    ...
    grad_x = cv2.Sobel(gray, cv2.CV_64F, 1, 0, ksize=3) #sobel算子计算梯度，这个算子跟intrinsicGS计算梯度的算子是一样的
    grad_y = cv2.Sobel(gray, cv2.CV_64F, 0, 1, ksize=3)
    grad_magnitude = cv2.magnitude(grad_x, grad_y) #magnitude = sqrt(x^2 + y^2)，常用来计算复数的模
    ...
    sampled_indices = np.random.choice(prob_map_flat.size, size=num_samples, p=prob_map_flat) #以prob_map_flat的值作为概率随机抽取像素索引
...
def compute_new_points_ids(frustum_points: torch.Tensor, new_pts: torch.Tensor,
    ...
    """ Having newly initialized points, decides which of them should be added to the submap.
        ...
        这个函数对每个new_pts计算了和frustum_points点中最近邻的8个点，统计8个点中小于radius的个数，如果8个点中没有
        小于radius的点，就返回这些离散点的索引，形状[m]
    Args:
        frustum_points: Point within a current frustum of the active submap of shape (N, 3)
        new_pts: New 3D Gaussian means which are about to be added to the submap of shape (M, 3)
        radius: Radius whithin which the points are considered to be neighbors
        device: Execution device
    Returns:
        Indicies of the new points that should be added to the submap of shape (m)
    """
    if frustum_points.shape[0] == 0:
        return torch.arange(new_pts.shape[0])
    if device == "cpu":
        pts_index = faiss.IndexFlatL2(3) #L2索引类，3是向量维度
    else:
    ...
    pts_index.add(frustum_points)#登记frustum_points，这是搜索的范围

    split_pos = torch.split(new_pts, 65535, dim=0)
    distances, ids = [], []
    for split_p in split_pos:
        distance, id = pts_index.search(split_p.float(), 8) #被分割的new_pts是需要搜索的点，在frustum_points中对new_pts的每一个点搜索前8个最近邻点，这里可以改成4,2,1试试效果
        distances.append(distance)
        ids.append(id) #多余删去
    distances = torch.cat(distances, dim=0) #[M,8],前8个最近邻点距离
    ids = torch.cat(ids, dim=0) #多余删去
    neighbor_num = (distances < radius).sum(axis=1).int() #[n]
    pts_index.reset()
    return torch.where(neighbor_num == 0)[0] #where返回一个包含两个tensor的元组，第一个是满足条件的元素的索引，第二个是满足条件的元素的值；返回值形状[m],m是new_pts中没有邻居点的数量，里面装的是满足条件的索引
...
def geometric_edge_mask(rgb_image: np.ndarray, dilate: bool = True, RGB: bool = False) -> np.ndarray:
    ...
    gray_image = cv2.cvtColor( #转灰度图
        rgb_image, cv2.COLOR_BGR2GRAY if not RGB else cv2.COLOR_RGB2GRAY)
    if gray_image.dtype != np.uint8:
        gray_image = gray_image.astype(np.uint8)
    edges = cv2.Canny(gray_image, threshold1=100, threshold2=200, apertureSize=3, L2gradient=True) # mask计算的时候可以修改这里的阈值，改变mask的密度
    # Define the structuring element for dilation, you can change the size for a thicker/thinner mask
    if dilate:
        kernel = np.ones((2, 2), np.uint8)
        edges = cv2.dilate(edges, kernel, iterations=1)#这个膨胀函数是使用卷积核让边缘变大
    return edges
...
def create_point_cloud(image: np.ndarray, depth: np.ndarray, intrinsics: np.ndarray, pose: np.ndarray) -> np.ndarray:
    ...
    u, v = np.meshgrid(np.arange(width), np.arange(height))#U:(height, width),V：(height, width)
    # Convert pixel coordinates to camera coordinates
    x = (u - intrinsics[0, 2]) * depth / intrinsics[0, 0]#x，y，z：(height, width)
    y = (v - intrinsics[1, 2]) * depth / intrinsics[1, 1]
    z = depth
    # Stack the coordinates together
    points = np.stack((x, y, z, np.ones_like(z)), axis=-1)  #points：(height*width, 4)，4的索引分别指的是x，y，z，1
    # Reshape the coordinates for matrix multiplication
    points = points.reshape(-1, 4)  #points：(height*width, 4)
    # Transform points to world coordinates
    posed_points = pose @ points.T  #points.T：(4, height*width) ；posed_points：(4,height*width)
    posed_points = posed_points.T[:, :3]  #posed_points：(height * width, 3) ，3分别指的是x，y，z
    # Flatten the image to get colors for each point
    colors = image.reshape(-1, 3)  # colors：(height * width, 3)
    # Concatenate posed points with their corresponding color
    point_cloud = np.concatenate((posed_points, colors), axis=-1)  #point_cloud：(height * width, 6)，6分别指的是x，y，z，r，g，b
