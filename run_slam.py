...
from src.entities.gaussian_slam import intrinsicGSlam
...
if __name__ == "__main__":
    ...
    if config["use_wandb"]:
        wandb.init(
            ...
            config=config,
            dir="/home/leehong/intrinsicGSlam/output/",
    ...
    igslam = intrinsicGSlam(config)
    igslam.run()

    # evaluator = Evaluator(igslam.output_path, igslam.output_path / "config.yaml")
    # evaluator.run()
    # if config["use_wandb"]:
    #     evals = ["rendering_metrics.json",
    #              "reconstruction_metrics.json", "ate_aligned.json"]
    #     log_metrics_to_wandb(evals, igslam.output_path, "Evaluation")
    #     wandb.finish()
    print("All done.✨")
